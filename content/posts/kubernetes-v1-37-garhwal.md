...
Whoever made this did the homework. The bird is a Himalayan [monal](https://en.wikipedia.org/wiki/Himalayan_monal), Uttarakhand's state bird. The red flowers are buransh, the state tree, with Kubernetes helms where the centres should be. The terraced fields climb the way they actually climb there. The house is marked १.३७ in Devanagari. It also animates: the light drains out of the valley about fifteen seconds in, holds at night for a while, then comes back by the end of a 37 second loop. The copy hosted here always runs it. The version on kubernetes.io switches it off if your system asks for reduced motion, which is a thoughtful touch on their part.
